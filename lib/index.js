 ...
 *     instruction-injection patterns) → register + per-source manifest;
...
/** Security review M2: after untar, reject the whole archive if any member is
 *  a symbolic link (file or directory). GNU tar preserves symlink members, so
 *  without this a crafted repository could plant links to arbitrary readable
 *  files (e.g. /etc/hostname, ~/.ssh) that earlier code would dereference into
 *  expert-sources/ and the merged view → model context. Runs on the staging
 *  copy, before anything is selected or registered. */
function assertNoSymlinks(dir, top = dir) {
  ...
  for (const name of readdirSync(dir)) {
    ...
    if (st.isSymbolicLink()) {
      throw new Error(`archive contains a symbolic link member: ${rel} — rejected (symlink-injection guard)`)
    }
    if (st.isDirectory()) assertNoSymlinks(full, top)
  }
}
...
export async function installSource(dst, sourceId, opts = {}) {
  ...
  try {
    ...
    assertNoSymlinks(staging) // M2: symlink members reject the whole archive
    ...
    if (usedChannel.pack) {
      ...
      const unpacked = readdirSync(staging).filter(n => statSync(join(staging, n)).isDirectory())
      if (unpacked.length === 1) root = join(staging, unpacked[0])
    }
    ...
    // 4) security scan before anything is registered
    const findings = scanSecurity(selected)
    if (findings.length) {
      const head = findings.slice(0, 20).map(f => `${f.file}:${f.line} [${f.kind}] ${f.name}`).join('\n  ')
      throw new Error(`security scan rejected ${sourceId} (${findings.length} finding(s)):\n  ${head}`)
    }
...
async function installCustomSource(dst, entry, channel, opts = {}) {
  ...
  if (!entry.upstream?.repo) {
    ...
    try {
      assertNoSymlinks(staging) // M2: symlink members reject the whole archive
    } catch (err) { cleanup(); throw err } // no staging leakage on the rejection path
    const unpacked = readdirSync(staging).filter(n => statSync(join(staging, n)).isDirectory())
...
async function remoteDownloadSourceLocked(dst, sourceId, channel, expectedRevision, opts = {}) {
  ...
  try {
    if (regEntry) {
      await installSource(dst, sourceId, { registry, channel, mirrorPrefixes })
    } else {
...
export default {
  ...
  apply(ctx, config = {}) {
    ...
    return { migration, targetDir: dst }
  },
}
