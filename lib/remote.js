 ...
 *   cleanupCustomDeleted(expectedRevision)                 → SourcesSnapshot (P7: purge soft-deleted)
...
import {
  ...
  remoteSaveCustom, remoteDeleteCustom, remoteCleanupCustomDeleted, getCustomExpert, remoteSetExpertEnabled,
...
function buildMethods() {
  return {
    ...
    // P7: purge soft-deleted custom records (no material change → revision
    // untouched). Settings-panel UI wiring is a deferred follow-up; the RPC
    // contract is complete so the panel can surface a "purge deleted" action.
    async cleanupCustomDeleted(expectedRevision) {
      return remoteCleanupCustomDeleted(resolveTargetDir(), expectedRevision)
    },
