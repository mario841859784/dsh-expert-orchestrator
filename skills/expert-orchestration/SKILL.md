...
- 逐个子任务召唤执行专家，选人按三级定向查找：① 先查本技能目录下的 `routing.md`（相对路径基于技能目录），按任务类型取首选/备选专家；② 再按专家 frontmatter（适用任务/禁入任务/典型交付）过滤确认匹配；③ 兜底才 `list_experts`（可按 division 过滤）全量浏览。routing.md 与本协议冲突时以 SKILL.md 为准。单个用 `summon_expert`，可并行的多个用 `summon_experts`（最多 8，并发 4）。
