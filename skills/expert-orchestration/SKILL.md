...
- 组版规则：模板按「固定段在前、变量段在后」组版——「核实」「红线」「产出落盘」三行逐字置前，目标/上下文/经验提示/要求/交付/完成定义六个变量段依次置后；字段顺序变化不丢语义，专家仍能看到全部字段；前部任何逐字差异都会截断公共前缀、令缓存失效。固定段内的 <你的专家名>/<并行协作时> 等占位符按当次召唤解析后保持逐字稳定。
- 相对路径一律明确说明基于当前工作目录。
